...
class SaveData:
    ...
    order_corrections_file = 'order_corrections.json'

    def __init__(self, customer_codes_file_path='customer_codes.json', orders_file_path='orders.json', warehouse_path='warehouse_data.json', order_times_file_path='order_times.json', order_corrections_file_path='order_corrections.json'):
        ...
        self.order_corrections_file_path = order_corrections_file_path
        self.order_corrections = self.load_order_corrections()
    ...
    @staticmethod
    def load_order_corrections():
        if os.path.exists(SaveData.order_corrections_file):
            with open(SaveData.order_corrections_file, 'r') as file:
                return json.load(file)
        return []

    @staticmethod
    def save_order_correction(correction):
        try:
            print("Attempting to save order correction...")  # Debug statement
            if not os.path.exists(SaveData.order_corrections_file):
                print(f"File {SaveData.order_corrections_file} does not exist, creating file...")  # Debug statement
                with open(SaveData.order_corrections_file, 'w') as file:
                    json.dump([], file, indent=4)
            
            # Load existing corrections
            order_corrections = SaveData.load_order_corrections()
            print(f"Current corrections loaded: {order_corrections}")  # Debug statement

            # Append the new correction
            order_corrections.append(correction)
            print(f"Appending correction: {correction}")  # Debug statement

            # Save the corrections back to the file
            with open(SaveData.order_corrections_file, 'w') as file:
                json.dump(order_corrections, file, indent=4)
            print(f"Order correction saved successfully: {order_corrections}")  # Debug statement
        except Exception as e:
            print(f"Error saving order correction: {e}")
